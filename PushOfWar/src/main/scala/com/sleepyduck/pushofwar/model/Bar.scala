...
object Bar extends AnyRef with Cost10

class Bar(pow: PushOfWarTest, x: Float = 0, y: Float = 0, angle: Float = 0, w: Float = 10, h: Float = 1, copied: Boolean = false)
	extends BaseObjectDynamic(pow, x, y, angle, copied) with Cost10 {

	def copy = new Bar(pow, body.getWorldCenter().x, body.getWorldCenter().y, body.getAngle(), w, h)
